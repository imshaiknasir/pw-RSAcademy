...
test('End to End Testing', { tags: ['endToEnd'] }, async ({ page }) => {
    ...
    for (let i = 0; i < (await countryDropdown.count()); i++) {
        ...
        if (countryText === testData.country) {
            console.log(
                `>>> Country text '${countryText}' matched with test data '${testData.country}' and we are clicking on it`,
            )
            await country.click()
            break
        }
    ...
    for (let i = 0; i < (await allOrderRows.count()); i++) {
        ...
        if (orderIdFromOrderHistory === extractedOrderId) {
            // this will check if the order id from the order history matches with the extracted order id
            console.log(
                `>>> Order ID from order history is '${orderIdFromOrderHistory}' and we are clicking on the view button`,
            )
